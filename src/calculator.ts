...
function err(text: string) {
	$("#info").text(text);
	var $result = $("#result");
	if (!$result.hasClass("none-display")) $result.addClass("none-display");
}

function calculate() {
	var weight = parseFloat($("#pers-info-weight").val() as string),
	height = parseFloat($("#pers-info-height").val() as string),
	gender = parseInt($("#pers-info-gender").val() as string), // 0女1男
	age = parseInt($("#pers-info-age").val() as string),
	bmi = weight * 10000 / height / height;
	if (isNaN(weight)) err("请输入正数体重");
	else if (isNaN(height)) err("请输入正数身高");
	else if (height < 1) err("身高不能小于1");
	else if (gender < 0 || gender > 1) err("请正确填写性别");
	else if (isNaN(age)) err("请输入正数年龄");
	else if (age < 7) err("请输入大于等于7的年龄");
	else {
		var $result = $("#result"),
		...
		if (bmi < dataStep[0]) {
			...
			if (progressSurp < 0) progressSurp = 0;
		}
			...
			if (progressSurp > 1) progressSurp = 1;
		}
		if ($result.hasClass("none-display")) $result.removeClass("none-display");
